...
void UKF::PredictLidarMeasurement(MatrixXd &Zsig, VectorXd &z_pred,
                                  MatrixXd &S) {
  // set measurement dimension, lidar can measure x and y coords
  int n_z = 2;
  ...
  if (!is_initialized_) {
    if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      x_ << meas_package.raw_measurements_[0],
          meas_package.raw_measurements_[1], 0, 0;
    }
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  NIS_laser_ = ((meas_package.raw_measurements_ - z_pred).transpose()) *
               S.inverse() * (meas_package.raw_measurements_ - z_pred);
}
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  UpdateState(meas_package.raw_measurements_, Zsig, z_pred, S);
  NIS_radar_ = ((meas_package.raw_measurements_ - z_pred).transpose()) *
               S.inverse() * (meas_package.raw_measurements_ - z_pred);
}
